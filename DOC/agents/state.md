...
- **最近校对（北京时间 GMT+8）**：2026-01-08（Frontend：套装模板编辑抽屉工作台化：抽屉加宽至 1280，改为左右两栏（左 1/3 短语列表：新建/复制/启停/选中；右 2/3 当前短语组件行编辑：模型/宽高/数量/筛选/复制/删除），避免“所有短语全展开”导致页面爆炸；验收 `npm -C frontend run build`）
