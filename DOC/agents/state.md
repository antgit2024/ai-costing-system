...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Frontend：套装模板中心新增 **“预设变体筛选”**（A 方案）：组件行可打开弹窗，按 base_line（物料行）分组列出该版本所有变体规则（priority/stop_on_hit/条件/替换物料），每行可选 0/1 条，并支持“一键按勾选规则填充 TOKEN(any/all) 触发词”到组件触发词输入框；同时保留严格多选触发词下拉（不支持自由输入）。验收：`npm -C frontend run build`、静态发布）
