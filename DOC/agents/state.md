...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Backend+Frontend：套装模板中心新增“公共触发词”能力：模板新增 `shared_trigger_text`（存入 metadata.shared_trigger_text），生成合并BOM时合并 tokens=交易规格解析tokens + 公共触发词tokens + 行级附加触发词tokens；前端抽屉新增“公共触发词（作用于所有组件）”输入框；验收：pytest、后端重启、`npm -C frontend run build`、静态发布）
