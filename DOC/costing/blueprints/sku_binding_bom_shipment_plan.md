...
## 5.1 交易规格（spec_text）变更：如何操作才不会把系统搞乱

你担心的点是正确的：spec_text 一旦变动，尺寸/Token/触发规则都会变；ERP 里必须把它当作“动态输入”，但要做到**可追溯、可重算、不影响已出库已核算**。

- **原则 1：快照不回写**
  - 已生成并用于扣库/核算的 `bom_snapshot` **永远不因规则/词典/规格变更而回写**
  - 重新解析/重新生成，只产生新的快照（用于后续出库/补发/重算分析），不改历史结果

- **原则 2：用 hash 触发重解析**
  - 对每条 `shipment_line` 记录 `spec_text`，并生成 `spec_hash`
  - 若同一个 `shipment_line`（同 external_line_key）再次出现，但 `spec_hash` 变化：
    - 生成新的 `spec_parse_snapshot` 与新的 `bom_snapshot`
    - 标记为 `revision`（修订），并保留“旧快照→新快照”的链路

- **原则 3：解析版本化**
  - `spec_parse_snapshot` 必须包含 `parser_version`
  - 当解析器/词典升级时：允许对“未出库/待处理队列”的行进行批量重解析，但**必须可回放**

## 5.2 历史几十万规格数据：临时规则对应关系怎么做（不堵死上线）

目标不是一步把所有历史都变成完美 Token，而是先把“能生成 BOM 快照”跑起来，再逐步固化治理。

### A) 建议的迁移策略（两条线并行）

- **线 1：SKU 绑定优先**
  - 先把“常见 sku_code（货品条码）→ published 标准版本”绑定起来（优先 Top 销量/Top 金额）
  - 绑定齐了，哪怕 spec_text 解析不完美，也能先走“兜底清单”出结果（再逐步加变体）

- **线 2：临时解析规则库（Template/Regex）**
  - 为历史 spec_text 建一个“临时规则库”（按店铺/品类/货品编号系列分组）：
    - 识别尺寸：`40*60cm`、`45X45`、`约50*70+30*30` 等
    - 识别关键触发词：如 `024画框`、`黑色包边`、`仿羊绒` 等
  - 临时规则库输出两类结果：
    - dimensions（宽/高/面积/周长）
    - tokens（先少量、可控，宁缺毋滥）

### B) “临时规则”如何逐步固化为运营规则

把临时规则库的高频命中项，逐步沉淀为：
- **全局 Token 词典**：同义词、规范名（运营可维护）
- **品类解析模板**：每个品类/系列一套“字段→解析策略”模板（减少自由格式）

## 5.3 新订单运营规则：让 spec_text 变“可控输入”

你提到“指定一些规则的字符”，建议做成运营可执行的约束（先软约束，后硬约束）：

- **软约束（立即可做）**
  - 给店铺运营一个“规格文本规范”：
    - 尺寸字段必须包含可解析格式（如 `尺寸:40*60cm` 或 `45X45`）
    - 关键触发词必须保留（画框型号/包边颜色/材质关键词）
    - 允许顺序变化，不允许删除关键段

- **硬约束（系统侧逐步加）**
  - 导入/同步时对 spec_text 运行校验：
    - 尺寸缺失/无法解析 → 进入异常队列，不直接算 BOM（避免脏数据进入扣库）
    - Token 变化幅度过大 → 提示需人工确认（审批/预演）
