...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Backend：修复扣库仍遗漏兜底物料：inventory 构建对 `material_kind` 做前缀兼容（real_material / virtual_material 等）+ 大小写兼容，避免历史脏数据被当成“未知类型”跳过；验收后端 py_compile+pytest（扣库单测新增覆盖 real_material），已重启后端）
