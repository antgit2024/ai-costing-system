/**
 * Finance C1 主数据只读展示页(派单 §2.7)。
 *
 * - 4 Tabs: 公司主体 / 店铺 / 员工 / 固定开支
 ...
 *
 * 不展示 metadata 字段(派单 §2.7"核心字段即可")· 但 Tooltip / Drawer 里
 * 给一个 "查看完整字段" 入口让 admin 调试时能看(后续如需可加)。
 *
...
function CompaniesTab() {
  const query = useQuery({
    queryKey: ['planner', 'finance-c1', 'companies'],
    ...
    () => [
      ...
      },
      {
        ...
        render: (v?: number | null) =>
          v != null ? (
            <Text style={{ fontFamily: 'monospace' }}>¥{v.toLocaleString()}</Text>
          ) : (
    ...
    <>
      <Card size="small" style={{ marginBottom: 12 }}>
        <Space size={16}>
          ...
          <Text type="secondary">
            合计: <Text strong>¥{grandTotal.toLocaleString()}</Text> · 共{' '}
...
function BadgeBar({ envelope }: BadgeBarProps) {
  ...
  return (
    <div style={{ marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {envelope.pagination && (
        <Text type="secondary" style={{ fontSize: 12 }}>
          · 共 {envelope.pagination.total} 条 (page {envelope.pagination.page}/
          {envelope.pagination.total_pages})
        </Text>
      )}
    </div>
  )
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        <div>
          ...
          <Paragraph type="secondary" style={{ marginBottom: 0 }}>
            来自 finance-analyzer 的 C1 契约(
            <Text code>finance_to_costing_c1_contract_v1.md</Text>)·
            5 类只读数据 · 5 分钟 TTL 缓存 + finance 挂时降级 ·
            数据维护在 finance-analyzer · 此页仅展示。
          </Paragraph>
        </div>
        <Space>
          <Tag color="green">C1 v1.0</Tag>
        </Space>
      </div>
