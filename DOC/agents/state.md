...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Backend+Frontend：套装模板口径修正：合并器按“单一交易规格”解析共享 tokens，并自动应用到所有组件（避免每组件 spec_text 误导且避免尺寸污染）；模板行 `spec_text` 文案改为“附加触发词（可选）”，仅用于补充变体触发（不填尺寸）；验收：pytest（bundle_templates CRUD）、后端重启、`npm -C frontend run build`、静态发布）
