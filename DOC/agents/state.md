...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Frontend：套装模板中心组件清单改为**每个模型三行展示**：第一行=版本/宽/高/数量/操作；第二行=“预设变体筛选”按钮；第三行=已选变体的“变体表达式 + 替换物料”摘要（未选则不显示）。同时移除原“触发词下拉勾选框”。结构位(slot)显示增加多字段兜底读取（metadata_json.structure_slot 等）。验收：`npm -C frontend run build`、静态发布）
