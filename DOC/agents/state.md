...
- **最近校对（北京时间 GMT+8）**：2026-01-22（天猫SKU生成器：移除“套装模板(B/Z)→天猫属性词”模块）
  - 本轮范围：清除天猫SKU生成器内不再使用的“套装模板（B/Z）→ 天猫属性词（占主动权）”整块能力，避免误导与维护负担。
  - 本轮产物：
    - 前端：`frontend/src/pages/costing/TmallSkuTemplateGeneratorPage.tsx`
      - 移除套装短码解析/拉取套装模板/互斥组组合生成颜色分类的 UI 与逻辑。
      - “模型属性”下拉仅保留已发布标准模型来源（不再加载套装模板）。
      - 移除与上述模块绑定的检验/公式渲染等遗留展示。
  - 验收命令（必须，全部 0 退出码）：
