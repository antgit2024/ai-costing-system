...
- **最近校对（北京时间 GMT+8）**：2026-01-07（Frontend：套装模板“短语预设→映射组”再收口：映射组去掉“字段(材质/枕芯)”列；列名改为“变体映射/目标模型”；每行操作改为“+行/删除”，取消上方“新增映射行”，新增短语默认带 1 行空映射；已硬验收 `npm -C frontend run build` 并原子发布）
