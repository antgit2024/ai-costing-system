...
- **最近校对（北京时间 GMT+8）**：2026-01-07（Backend：支持套装编码选择器 `(B:CODE:A/B/C...)`：A=第1条短语预设、B=第2条…（不再依赖包含命中）；已补测试 `backend/tests/planner/test_bom_generate_by_spec_bundle_selector.py` 并通过；同时修复后端 import 级缩进错误：`schemas.py` 与 `routers/base_config.py` 可编译）
