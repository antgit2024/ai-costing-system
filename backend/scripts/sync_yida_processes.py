#!/usr/bin/env python3
"""YiDa process/labor sync CLI helper."""
...
from src.planner.services.yida_sync import ProcessSyncService, YidaConfig, YidaFormClient


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sync YiDa processes/labor data into costing database.")
...
def main() -> None:
    ...
    config_path = Path(args.config) if args.config else Path(settings.yida_processes_config_path)
...
def run_sync(
    ...
    *,
    ...
    service = ProcessSyncService(session, client)
    return service.sync_processes(limit=limit, dry_run=dry_run, dump_path=dump_path)
