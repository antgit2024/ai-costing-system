...
| **U2** | 校准 `finance_analyzer_integration_v1.md` v1.1 → v1.2 | (a) 用户启动 finance 集成 / monthly_cost_variance / 多主体合并 (b) 用户提及 finance_analyzer 文档 (c) Phase 1 W4 开始（finance 录入） | 🟢 **2026-05-10 09:35 costing 侧 C1 client 完成**（等 finance 侧实施）：派单 `briefings/costing_c1_client_service.md` 全栈交付 — `finance_c1_client.py`（5 方法 + TTLCache + _last_good 兜底 + 失败降级矩阵）+ `finance_c1_schemas.py`（5 DTO + `FinanceC1ListEnvelope`）+ `tests/mocks/finance_c1_mock.py`（7/4/3/11/6 按契约 §3 1:1）+ `routers/finance_c1_proxy.py`（5 个 GET 挂 `/api/planner/finance/*`）+ `FinanceMasterDataPage.tsx`（4 Tabs + 数据来源 Badge 🟢/🟡/🔵/🔴）+ 路由 `/costing/admin/finance-master`。验收 B1~B5 + F1~F4 共 9/9 全过；12 单测全过 + 0 回归 + frontend build OK + F4 BOM/Hub/Insights 零改动（git diff 0 lines）。**U2 完成定义剩 2 件**：(1) finance-analyzer 侧 §2 5 个 GET API 落地 + §4.1 5 条自测 + key 通过安全渠道发给 costing；(2) 双方跑通 §4.3 E2E1~E2E3 3 条；之后 ai-costing 切 `FINANCE_C1_USE_MOCK=false` 走真接口。**先前阶段**：2026-05-10 07:40 C1 契约规格书 v1.0 落地为 `finance_to_costing_c1_contract_v1.md`（master_companies canonical + 5 API + C2 测试 + C3 SLA + 13 条验收）。`finance_analyzer_integration_v1.md` v1.1→v1.2 架构层校准延后（按减法工作流，等 finance 实施落地后一起做）| C1 契约 v1.0 上线 + finance 侧 §8.1 5 条 + costing 侧 §8.2 5 条全过 |
