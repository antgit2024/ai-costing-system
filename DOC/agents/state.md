...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Frontend：套装模板中心“预设变体筛选”弹窗默认列表改为**只看结果**：折叠标题显示“结构位：原物料 + 规则数 +（已选时）替换物料”，不在列表显示 TOKEN/条件表达式；展开后仍可查看规则细节。验收：`npm -C frontend run build`、静态发布）
