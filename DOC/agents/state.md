...
- **最近校对（北京时间 GMT+8）**：2026-01-07（Frontend：套装模板继续去噪：示例规格自动输出为“短语；(B:CODE)”并避免重复拼接；组件行将“预设变体筛选”按钮移到操作列、改名“筛选”且放在删除前；删除多余说明文案；已硬验收 `npm -C frontend run build` 并原子发布）
