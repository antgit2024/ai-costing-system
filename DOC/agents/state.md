...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Backend+Frontend：测试台 UI 升级：左侧改为 TAB「单模型测试/多模型测试」。多模型测试每行可选择不同 `model_version_id` + 尺寸/数量 + 交易规格(特征串，仅用于 tokens)，后端新增 `/api/planner/bom/generate-multi-bundle` 按版本分组生成并总合并；验收 `npm -C frontend run build`，已发布静态并重启后端）
