...
| **U7** | 4 个 Insights 看板加成本可信度徽章 + ~~三视图切换~~"按店铺法人主体切片"维度 | (a) 与 U5/U6 联动 (b) Phase 1 W4-W5 开始 | ✅ **A 部分完成**（`badges_mvp.md` 2026-05-09 全栈交付）；**B 部分定义已纠正（2026-05-10 15:35）**：原"三视图 Tax/Mgmt/Group"是误读 — 税务账 + 集团合并归 finance 域（已有 3 套报表，不重复造），costing 只做管理账（这就是 Insights 看板本身），增量是**给 4 看板加"按店铺法人主体（4 家店铺单独）"筛选维度**（0.5~1 天工作量，并入路径 A 的 A5 步骤一起做） | A 部分：5 条用户验收标准全过；B 部分：路径 A 的 A5 步骤里顺手做（不再单独派 brief）|
