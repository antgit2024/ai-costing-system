...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Frontend：套装字符映射编辑器交互统一：移除顶部“新增规则”，改为在规则表“操作”列提供“+行/命中/删除”；“命中”会弹出测试片段输入框，快速验证该规则是否能命中并提示分配到哪个组件 label。验收：`npm -C frontend run build`、静态发布）
