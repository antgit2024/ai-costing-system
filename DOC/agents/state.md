...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Frontend：套装模板中心“附加触发词”升级为严格多选下拉：组件行选择模型版本后，下拉候选来自该版本启用变体规则的 TOKEN(any/all) 提取；**不支持自由输入**，避免“以为能触发但实际触发不了”；同时更新文案：运营写尺寸不影响模板尺寸解析（这里只用于触发词）。验收：`npm -C frontend run build`、静态发布）
