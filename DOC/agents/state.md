...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Backend：修复扣库遗漏真实兜底物料：inventory 构建对 `material_kind` 做大小写兼容（REAL/real/Virtual 等），避免历史脏数据被当成“未知类型”跳过；验收后端 py_compile+pytest（扣库单测新增覆盖 REAL），已重启后端）
