...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Frontend：清理测试台残留：单模型测试 TAB 仅保留 spec/parse + bom/generate，移除之前误放入的“套装/多行组件/套装明细”；多模型测试 TAB 清理单模型复制残留（版本候选/模型版本摘要等），右侧诊断 TAB 按模式精简；验收 `npm -C frontend run build`，已发布静态）
