...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Backend+Frontend：修复多模型合并扣库清单缺失：扣库聚合不再因缺 `material_code` 丢行（fallback 到 material_id/名称），并补齐合并键的 `unit_of_measure`；多模型合并输出与前端扣库区块新增 warnings 展示，便于定位“虚拟物料未绑定/物料缺编码”等原因；验收后端 py_compile+pytest（新增扣库单测），前端 `npm -C frontend run build`，已发布静态并重启后端）
