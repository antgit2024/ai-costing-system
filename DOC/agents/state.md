...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Frontend：套装模板中心新增“变体触发提醒”：当组件行选择某个模型版本时，自动拉取该版本启用的变体规则，若存在依赖 Token 的条件（TOKEN any/all），则在该行提示“可能需要触发词”并展示示例词，避免运营忘写交易规格触发词导致落入兜底；验收：`npm -C frontend run build`、静态发布）
