...
- **最近校对（北京时间 GMT+8）**：2026-01-06（Backend+Frontend：新增长期栏目“套装模板”：后端 `bundle_templates` 管理API补齐（列表/详情/编辑/复制/归档，支持 metadata.category/tags 过滤；组件入库时将 Decimal 序列化为 JSON 基元）；前端新增页面 `/costing/bundle-templates`（列表筛选+抽屉编辑组件清单+复制/归档）并加入左侧菜单；验收：pytest（bundle_templates CRUD）、后端重启、`npm -C frontend run build`、静态发布）
