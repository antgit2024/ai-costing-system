...
- **最近校对（北京时间 GMT+8）**：2026-01-07（Frontend：套装模板编辑页进一步收口：移除旧的“全局字段映射/可变词列表”等易误会区块，仅保留“短语预设→映射组”的主路径；保存时保留历史 `metadata.lexicon_rules` 不误删；已硬验收 `npm -C frontend run build` 并原子发布）
